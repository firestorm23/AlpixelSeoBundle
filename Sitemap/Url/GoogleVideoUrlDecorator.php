...
class GoogleVideoUrlDecorator extends UrlDecorator
{
    ...
    public function __construct(Url $urlDecorated, $thumnail_loc, $title, $description, array $parameters = [])
    {
        ...
        if (!$this->content_loc && !$this->player_loc) {
            throw new \Exception('The parameter content_loc or player_loc is required');
        }

        if (count($this->platforms) && !$this->platform_relationship) {
            throw new \Exception('The parameter platform_relationship is required when platform is set');
        }
    ...
    public function setDuration($duration)
    {
        if (!is_numeric($duration) || $duration < 0 || $duration > 28800) {
            throw new \Exception(sprintf('The parameter %s must be a valid duration.see http://support.google.com/webmasters/bin/answer.py?hl=en&answer=80472#4', $duration));
        }
    ...
    public function setRating($rating)
    {
        if (!is_numeric($rating) || $rating < 0 || $rating > 5) {
            throw new \Exception(sprintf('The parameter %s must be a valid rating.see http://support.google.com/webmasters/bin/answer.py?hl=en&answer=80472#4', $rating));
        }
    ...
    public function setViewCount($view_count)
    {
        if (!is_int($view_count)) {
            throw new \Exception(sprintf('The parameter %s must be a valid view count.see http://support.google.com/webmasters/bin/answer.py?hl=en&answer=80472#4', $view_count));
        }
    ...
    public function setCategory($category)
    {
        if (strlen($category) > 256) {
            throw new \Exception(sprintf('The parameter %s must be a valid category. see http://support.google.com/webmasters/bin/answer.py?hl=en&answer=80472#4', $category));
        }
    ...
    public function addTag($tag)
    {
        if (count($this->tags) >= self::TAG_ITEMS_LIMIT) {
            throw new \Exception(sprintf('The parameter %s must be a valid family_friendly. see http://support.google.com/webmasters/bin/answer.py?hl=en&answer=80472#4'));
        }
